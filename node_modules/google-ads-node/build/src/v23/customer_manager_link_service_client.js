 ...
 * `src/v23/customer_manager_link_service_client_config.json`.
...
class CustomerManagerLinkServiceClient {
     ...
     *     const client = new CustomerManagerLinkServiceClient({fallback: true}, gax);
     *     ```
     */
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.ads.googleads.v23.services.CustomerManagerLinkService', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        this.customerManagerLinkServiceStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.ads.googleads.v23.services.CustomerManagerLinkService') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.ads.googleads.v23.services.CustomerManagerLinkService, this._opts, this._providedCustomServicePath);
    ...
    mutateCustomerManagerLink(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('mutateCustomerManagerLink response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.mutateCustomerManagerLink(request, options, wrappedCallback)
    ...
    moveManagerLink(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('moveManagerLink response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.moveManagerLink(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('moveManagerLink response %j', response);
            return [response, options, rawResponse];
        }).catch((error) => {
