 ...
 * `src/v23/account_link_service_client_config.json`.
...
class AccountLinkServiceClient {
     ...
     *     const client = new AccountLinkServiceClient({fallback: true}, gax);
     *     ```
     */
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.ads.googleads.v23.services.AccountLinkService', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        this.accountLinkServiceStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.ads.googleads.v23.services.AccountLinkService') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.ads.googleads.v23.services.AccountLinkService, this._opts, this._providedCustomServicePath);
    ...
    createAccountLink(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('createAccountLink response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.createAccountLink(request, options, wrappedCallback)
    ...
    mutateAccountLink(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('mutateAccountLink response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.mutateAccountLink(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('mutateAccountLink response %j', response);
            return [response, options, rawResponse];
        }).catch((error) => {
