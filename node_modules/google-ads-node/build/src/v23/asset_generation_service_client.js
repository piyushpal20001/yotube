...
exports.AssetGenerationServiceClient = void 0;
const jsonProtos = require("../../protos/protos.json");
const google_gax_1 = require("google-gax");
/**
 * Client JSON configuration object, loaded from
 * `src/v23/asset_generation_service_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
const gapicConfig = require("./asset_generation_service_client_config.json");
...
class AssetGenerationServiceClient {
    ...
    innerApiCalls;
    pathTemplates;
    assetGenerationServiceStub;
    /**
     * Construct an instance of AssetGenerationServiceClient.
     *
     ...
     *     const client = new AssetGenerationServiceClient({fallback: true}, gax);
     *     ```
     */
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.ads.googleads.v23.services.AssetGenerationService', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        if (this.assetGenerationServiceStub) {
            return this.assetGenerationServiceStub;
        }
        // Put together the "service stub" for
        // google.ads.googleads.v23.services.AssetGenerationService.
        this.assetGenerationServiceStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.ads.googleads.v23.services.AssetGenerationService') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.ads.googleads.v23.services.AssetGenerationService, this._opts, this._providedCustomServicePath);
        // Iterate over each of the methods that the service provides
        // and create an API call method for each.
        const assetGenerationServiceStubMethods = ['generateText', 'generateImages'];
        for (const methodName of assetGenerationServiceStubMethods) {
            const callPromise = this.assetGenerationServiceStub.then(stub => (...args) => {
                if (this._terminated) {
    ...
    generateImages(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        this.initialize().catch(err => { throw err; });
        this._log.info('generateImages request %j', request);
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('generateImages response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.generateImages(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('generateImages response %j', response);
            return [response, options, rawResponse];
        }).catch((error) => {
     ...
     */
    close() {
        if (this.assetGenerationServiceStub && !this._terminated) {
            return this.assetGenerationServiceStub.then(stub => {
...
exports.AssetGenerationServiceClient = AssetGenerationServiceClient;
//# sourceMappingURL=asset_generation_service_client.js.map