...
exports.IncentiveServiceClient = void 0;
const jsonProtos = require("../../protos/protos.json");
const google_gax_1 = require("google-gax");
/**
 * Client JSON configuration object, loaded from
 * `src/v23/incentive_service_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
const gapicConfig = require("./incentive_service_client_config.json");
...
class IncentiveServiceClient {
    ...
    /**
     * Construct an instance of IncentiveServiceClient.
     *
     ...
     *     const client = new IncentiveServiceClient({fallback: true}, gax);
     *     ```
     */
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.ads.googleads.v23.services.IncentiveService', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        if (this.incentiveServiceStub) {
            return this.incentiveServiceStub;
        }
        // Put together the "service stub" for
        // google.ads.googleads.v23.services.IncentiveService.
        this.incentiveServiceStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.ads.googleads.v23.services.IncentiveService') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.ads.googleads.v23.services.IncentiveService, this._opts, this._providedCustomServicePath);
        // Iterate over each of the methods that the service provides
        // and create an API call method for each.
        const incentiveServiceStubMethods = ['fetchIncentive', 'applyIncentive'];
        for (const methodName of incentiveServiceStubMethods) {
            const callPromise = this.incentiveServiceStub.then(stub => (...args) => {
                if (this._terminated) {
    ...
    fetchIncentive(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        ...
        this._log.info('fetchIncentive request %j', request);
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('fetchIncentive response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.fetchIncentive(request, options, wrappedCallback)
    ...
    applyIncentive(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        options.otherArgs.headers['x-goog-request-params'] = this._gaxModule.routingHeader.fromParams({
            ...
            'selected_incentive_id': request.selected_incentive_id?.toString() ?? '',
        });
        this.initialize().catch(err => { throw err; });
        this._log.info('applyIncentive request %j', request);
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('applyIncentive response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.applyIncentive(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('applyIncentive response %j', response);
            return [response, options, rawResponse];
        }).catch((error) => {
     ...
     */
    close() {
        if (this.incentiveServiceStub && !this._terminated) {
            return this.incentiveServiceStub.then(stub => {
...
exports.IncentiveServiceClient = IncentiveServiceClient;
//# sourceMappingURL=incentive_service_client.js.map